...
impl Common {
    fn get_test_app_metadata() -> AppMetadata {
        AppMetadata {
            ...
            desc: "An app for testing purpose".to_string(),
...
fn test_add_category() {
    let mut config = Config::new();
    config.add_category("test_category").unwrap();
    let category_metadata = config.get_category("test_category");
    assert!(category_metadata.is_some());
...
fn test_remove_app() {
    ...
    config.remove_app("test_app").unwrap();
    let app_metadata = config.get_app("test_app");
    assert!(app_metadata.is_none());
    
    let result = config.remove_app("non_existent_app");
    let e = result.expect_err("Expect error");
...
fn test_remove_category() {
    let mut config = Common::get_test_config();
    config.remove_category("test_category").unwrap();
    let category = config.get_category("test_category");
    assert!(category.is_none());
    
    let result = config.remove_category("non_existent_category");
    let e = result.expect_err("Expect error");
...
fn test_add_app_to_category() {
    ...
    let mut e;
    let mut config = Common::get_test_config();
    config.add_app("test_app_2", Common::get_test_app_metadata()).unwrap();
    ...
    config.add_app_to_category("test_app_2", "test_category").unwrap();
    let category_metadata = config.get_category("test_category").unwrap();
    assert_eq!(category_metadata.apps,  vec!["test_app".to_string(), "test_app_2".to_string()]);
}
...
fn test_remove_app_from_category() {
    ...
    result = config.remove_app_from_category("test_app_2", "test_category");
    e = result.expect_err("Expect error");
    ...
    assert_eq!(app_name, "test_app_2");
    assert_eq!(category_name, "test_category");

    config.remove_app_from_category("test_app", "test_category").unwrap();
    let category_metadata = config.get_category("test_category").unwrap();
    assert_eq!(category_metadata.apps.len(), 0);
}

#[test]
fn test_update_categories() {
    let mut config = Common::get_test_config();
    config.add_category("test_category_2").unwrap();
    config.update_categories(vec!["test_category_2".to_string(), "test_category".to_string()]).unwrap();
    assert_eq!(config.get_category_list(), vec![&"test_category_2".to_string(), &"test_category".to_string()]);
    let result = config.update_categories(vec!["test_category".to_string(), "non_existent_category".to_string()]);
    let e = result.expect_err("Expect error");
    let ConfigErrorType::CategoryNotExist(category_name) = e.err_type else {
        panic!("Expect CategoryNotExist, got {:?}", e.err_type);
    };
...
fn test_rename_category() {
    let mut config = Common::get_test_config();
    config.rename_category("test_category", "test_category_renamed").unwrap();
    assert!(config.get_category("test_category_renamed").is_some());
    assert!(config.get_category("test_category").is_none());
    let result = config.rename_category("non_existent_category", "new_category");
    let e = result.expect_err("Expect error");
    let ConfigErrorType::CategoryNotExist(category_name) = e.err_type else {
        panic!("Expect CategoryNotExist, got {:?}", e.err_type);
    };
...
#[test]
fn test_update_apps_in_category() {
    let mut config = Common::get_test_config();
    config.add_app("test_app_2", Common::get_test_app_metadata()).unwrap();
    config.update_apps_in_category(vec!["test_app_2".to_string(), "test_app".to_string()], "test_category").unwrap();
    let category_metadata = config.get_category("test_category").unwrap();
    assert_eq!(category_metadata.apps, vec!["test_app_2".to_string(), "test_app".to_string()]);
    let result = config.update_apps_in_category(vec!["test_app".to_string(), "non_existent_app".to_string()], "test_category");
    let e = result.expect_err("Expect error");
    let ConfigErrorType::AppNotExist(app_name) = e.err_type else {
        panic!("Expect AppNotExist, got {:?}", e.err_type);
    };
